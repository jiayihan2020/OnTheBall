...
# --- User inputs---
# Set the current working directory. By default, it is located to the folder containing this python script.
working_directory = "./"
# Input the sleep diary csv filename. By default, it uses wildcards to find.
input_csv = "SIT Diary Tri 3*.csv"
# List the subject codes of all the participants in the study.
all_participants = [
    "SIT049",
    "SIT051",
    "SIT054",
...
# Set the threshold day to determine when to classify students as not compliant to the Sleep Diary. Default number of days is 3.
threshold_day = 3
...
def extract_count():
    """Detect how many respondents have responded to the survey, and the total responses obtained for each respondents, and the grant total.
...
def threshold():
    ...
    Output: text file if there are non-compliant students detected.
    """
    df = opening_sleep_diary(input_csv)
    ...
    three_days_before = today_date - datetime.timedelta(days=threshold_day)
    ...
    if no_balls:
        print(
            "Students that are not on the ball detected. Exporting their Subject Code..."
        )
        with open(
